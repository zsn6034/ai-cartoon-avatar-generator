...
@app.post("/api/outfit/analyze/image", response_model=OutfitAnalysisResponse)
async def analyze_outfit_image(llm_config: str = Form(...), image: UploadFile = File(...)):
    ...
    selected_provider = get_provider(config)
    data_url = await image_to_data_url(image)
    result = await selected_provider.analyze_outfit_image(data_url)
    logger.info(
...
@app.post("/api/outfit/chat/remember", response_model=OutfitChatRememberResponse, response_model_exclude_none=True)
async def remember_outfit_chat(request: OutfitChatRememberRequest):
    ...
    result = await selected_provider.remember_outfit_chat(
        messages=messages,
        current_memory=current_memory,
    )
    logger.info(
...
@app.post("/api/outfit/chat/generate", response_model=OutfitAnalysisResponse)
async def generate_outfit_chat(request: OutfitChatGenerateRequest):
    ...
    result = await selected_provider.generate_outfit_chat(
        messages=messages,
        chat_memory=chat_memory,
    )
    logger.info(
